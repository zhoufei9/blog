...
class FocusController extends ApiController {
	...
	public function getArt($uid, $status, $order, $field = true, $row, $position, $limit) {
		...
		$count = D('Blog/Article')->where($map)->count();
		if ($count > 0) {
			$p = I(C('VAR_PAGE'));
			if ($limit) {
				$data = D('Blog/Article')->where($map)->order($order)->limit($row)->field($field)->select();
			} else {
				$data = D('Blog/Article')->where($map)->order($order)->page(!empty($p) ? $p : 1, $row)->field($field)->select();
			}
		}
		if ($data == null) {
	...
	public function getArtCount($uid, $status, $position) {
		...
		$map['id'] = array('in', $rowarr);
		$map['status'] = array('in', $status);
		$data = D('Blog/Article')->where($map)->count();
		if ($data == null) {
