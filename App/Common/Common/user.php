...
function query_user($fields, $uid = null)
{
    ...
    if ($homeFields) {
        $homeResult = D('Blog/Member')->where(array('uid' => $uid))->field($homeFields)->find();
    }
    if ($ucenterFields) {
   ...
   $ucenterResult['nickname']=op_t($ucenterResult['nickname']);
    ...
    if (in_array('rank_link', $fields)) {
     ...
     if (in_array('signature', $fields)) {
     	$result['signature']=D('Blog/Member')->where(array('uid' => $uid))->field($homeFields)->getField('signature');
