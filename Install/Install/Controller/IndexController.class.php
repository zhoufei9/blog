...
class IndexController extends Controller {
    ...
    public function index(){
        ...
        $jianjie_html =  $this->fetch(MODULE_PATH.'/View/public/jianjie.html');
    ...
    public function setconf() {
        ...
        if ($_POST) {
            ...
            if ($selected_db) {
                ...
                $query = $conn->query("SHOW TABLES LIKE '{$db_prefix}%'");
                $was_install = false;
    ...
    public function finish_done() {
        ...
        $conn = mysqli_connect($temp_info['db_host'] . ':' . $temp_info['db_port'], $temp_info['db_user'], $temp_info['db_pass']);

        $charset = str_replace('-', '', $charset);
        $conn->query("SET character_set_connection={$charset}, character_set_results={$charset}, character_set_client=binary");
        $conn->query("SET sql_mode=''");
        $selected_db = $conn->select_db($temp_info['db_name']);
        //开始创建数据表
